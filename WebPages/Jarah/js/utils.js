...
/**
 * Applies configuration values to the document, like setting CSS variables.
...
export function applyConfigToDOM(config) {
    ...
    // تنظیم URL فونت‌ها به عنوان متغیرهای CSS
    const englishFontUrl = `url('${rawBaseUrl}/${config.paths.englishFont}')`;
    const arabicFontUrl = `url('${rawBaseUrl}/${config.paths.arabicFont}')`;

    root.style.setProperty('--english-font-url', englishFontUrl);
    root.style.setProperty('--arabic-font-url', arabicFontUrl);
}
...
/**
 * Decodes an obfuscated token back to its original form.
 * @param {string} obfuscatedToken - The reversed Base64 token.
 * @returns {string} The original token.
 */
export function decodeObfuscatedToken(obfuscatedToken) {
    try {
        // الگوریتم دیکود:
        // 1. معکوس کردن رشته
        // 2. دیکود از Base64
        const reversedToken = obfuscatedToken.split('').reverse().join('');
        const realToken = atob(reversedToken);
        return realToken;
    } catch (e) {
        console.error("Failed to decode token. Is it correctly obfuscated?", e);
        return ""; // در صورت خطا، یک رشته خالی برمی‌گرداند تا از خطاهای بعدی جلوگیری شود
    }
}
